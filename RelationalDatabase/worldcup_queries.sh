...
echo "$($PSQL "SELECT name FROM games FULL JOIN teams ON games.winner_id=teams.team_id OR games.opponent_id=teams.team_id WHERE year=2014 AND round LIKE 'Eighth-Final' ORDER BY name ASC")"
...
echo "$($PSQL "SELECT name FROM teams WHERE name ILIKE'Co%' ORDER BY name ASC")"
